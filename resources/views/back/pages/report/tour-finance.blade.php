            ...
            <div class="card-body pt-0">
                <div class="row mb-10">
                    <div class="col-md-4">
                        ...
                        <label class="form-label fs-6 fw-bold">Paket Tour</label>
                        <select class="form-select form-select-solid" data-control="select2"
                            data-placeholder="Select an option" name="schedule_id" id="schedule_id">
                            <option value="1">Semua Paket</option>
                            @foreach ($tour_packages as $packages)
                                <optgroup label="{{ $packages->name }}">
                                    @foreach ($packages->schedules as $schedule)
                                        <option value="{{ $schedule->id }}">{{ $schedule->name }} -
                                            {{ $schedule->departure }}</option>
                                    @endforeach
                                </optgroup>
                            @endforeach
                        </select>
                        ...
                        <div class="card mb-5">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    ...
                                    <span class="fs-5 fw-bold text-success fs-2" id="total_income">Rp 10.000.000</span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card mb-5">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    ...
                                    <span class="fs-5 fw-bold text-danger fs-2" id="total_expense">Rp 53.753.453</span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card text-center">
                            <div class="card-body">
                                <div class="d-flex  align-items-center">
                                    ...
                                    <span class="fs-5 fw-bold fs-2  text-danger " id="balance">Rp -43.753.453 </span>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            var table = $('#table_finance').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('back.report.tour.finance.datatables') }}",
                    data: function(d) {
            ...
            table.on('xhr', function() {
                ...
                $('#export_excel').attr('href',
                    "{{ route('back.report.tour.finance.export') }}?schedule_id=" +
