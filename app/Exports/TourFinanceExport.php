...
class TourFinanceExport implements FromCollection, WithHeadings, WithStyles, WithEvents, WithCustomStartCell
{
    ...
    public function collection()
    {
        $data = TourFinance::with('tourSchedule', 'createdBy', 'updatedBy')
            ->when($this->schedule_id && $this->schedule_id != 1, function ($query) {
                return $query->where('tour_schedule_id', $this->schedule_id);
            })
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('A1', 'Data Keuangan Tour ' . Carbon::parse($this->date_start)->format('d M Y') . ' - ' . Carbon::parse($this->date_end)->format('d M Y'));
