...
use App\Exports\TourFinanceExport;
...
class ReportController extends Controller
{
    ...
    public function tourFinance()
    {
        $data = [
            'title' => 'Laporan Keuangan Tour',
            'breadcrumbs' => [
                ['name' => 'Dashboard', 'link' => route('back.dashboard.index')],
                ['name' => 'Laporan Keuangan Tour', 'link' => route('back.report.tour.finance')],
            ],
            'tour_packages' => TourPackage::with('schedules')->get(),
        ];

        return view('back.pages.report.tour-finance', $data);
    }

    public function tourFinanceDatatables(Request $request)
    {
        $schedule_id = $request->schedule_id;
        $date_end = $request->date_end ?? now()->toDateString();
        $date_start = $request->date_start ?? now()->subMonth()->toDateString();

        $data = TourFinance::with('tourSchedule', 'createdBy', 'updatedBy')
            ->when($schedule_id && $schedule_id != 1, function ($query) use ($schedule_id) {
                return $query->where('tour_schedule_id', $schedule_id);
            })
            ->whereBetween('date', [$date_start, $date_end])
            ->get();
    ...
    public function tourFinanceExport(Request $request)
    {
        $schedule_id = $request->schedule_id;
        $date_end = $request->date_end ?? now()->toDateString();
        $date_start = $request->date_start ?? now()->subMonth()->toDateString();

        return Excel::download(new TourFinanceExport($schedule_id, $date_start, $date_end), 'laporan-keuangan-tour.xlsx');
    }
}
